<?php 
  ...
  require '../00_config/connect.php';//db connect
	try{
		...
		$stmt = $pdo->prepare("SELECT user_id, user_name FROM 01_user_profile WHERE user_name=:user_name AND user_password=:hash_password AND (user_active = 1) AND (user_trash = 0)"); 
